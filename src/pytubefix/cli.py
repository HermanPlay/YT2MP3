...
def download_audio(
    youtube: YouTube, filetype: Optional[str] = "mp4", target: Optional[str] = None
) -> None:
    ...
    stream = (
        youtube.streams.filter(progressive=False, subtype=filetype)
        .order_by("abr")
        .last()
    )
    if stream is None:
...
def download_caption(
    youtube: YouTube, lang_code: str, target: Optional[str] = None
) -> None:
...
def main():
    ...
    setup_logger(
        logging.DEBUG if args.verbose else logging.INFO, log_filename=log_filename
    )
